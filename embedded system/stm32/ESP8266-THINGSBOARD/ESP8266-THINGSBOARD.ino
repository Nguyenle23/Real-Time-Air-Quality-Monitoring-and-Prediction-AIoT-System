...
const int port = 8080;
const char* accessToken = "ha15d2f43lrrg4au5ljx";
...
String stringFormat = "90,95,99", temp, humi, mq135, mq7, pm25, uv_index;
int a;
byte moc;
...
void loop() {
  ...
  if ((unsigned long)(millis() - time_post) > 10000) {
    ...
    String inputString = "29.7,46.0,0.72296,2.01194,0.56211,1.95312";
    int index = 0;

    // Define the keys for each value
    const char* keys[] = { "temperature", "humidity", "mq135", "mq7", "pm2.5", "uv_index" };

    // Tokenize the input string based on the comma delimiter
    char* value = strtok(const_cast<char*>(stringFormat.c_str()), ",");
    while (value != NULL && index < 6) {
